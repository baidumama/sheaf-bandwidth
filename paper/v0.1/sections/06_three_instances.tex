...
$35$ MRI configurations (\cref{subsec:6-topsp}), Instance~2 by the
...
\paragraph{Empirical evidence on real MRI data.}
A simulation suite developed in the present project applied
$\sigma_{\min}(A^{\TopSP})/\sigma_{\max}(A^{\TopSP})$ to $35$
configurations covering five trajectory families (Cartesian, random,
sparkling, spiral, radial) and seven $V_s$ sizes ($M$ ranging from
$30$ to $400$, with base scale $N = 200$). All $35$ configurations
attained full column rank, with a clear ordering: Cartesian sampling
at $\sheafBW \approx 0.0091$ versus radial sampling at $\sheafBW
\approx 2.07 \times 10^{-5}$, separating the two regimes by more
than two orders of magnitude. The
ordering matches the reconstruction quality reported in MRI sampling
...
\midrule
Empirical & b3pre numerical study ($35$ configs, multi-$N$ robustness) & this paper, \cref{sec:numerical}      & strong \\
